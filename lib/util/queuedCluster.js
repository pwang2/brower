...
function QueuedCluster(worker, concurrentNo, args, options, timeout) {
    ...
    this.pool = {};
    this.status = 'pending';
    this.concurrentNo = concurrentNo || process.env.WORKER_COUNT || cpus.length;
    var that = this;
...
util.inherits(QueuedCluster, EventEmitter);
...
QueuedCluster.prototype.enqueue = function(messages) {
    ...
    if (this.queue.length > QUEUE_LIMIT) {
        console.warn(that.queue.length + ' items in queue, think about scaling');
    }
