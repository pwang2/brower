...
var processDeps = require('../util/parseDeps.js'),
    flatDependencies = processDeps.flatDependencies;
...
var prefetchPromise = function(item) {
    ...
    console.log('xxxxxxx!!!!!', prefetchMsg);
    console.log('trigger prefetch for %s', prefetchMsg);
    return prefetchCluster.enqueue(prefetchMsg);
};
*/

var processMainFiles = function(metaObj, failedDeps, logger) {
    //run flat here to get the simple list for file copy,
    //later, in order to instrument Akami SHA which depends on file copy,
    //we need to flat again.
    var flatDep = flatDependencies(metaObj, logger);
    var copyAll = _.map(flatDep, function(component) {
        return copyComponentPromise(component, failedDeps);
    });
    var prefetchAll = [];
    //disable this due to performance issue
