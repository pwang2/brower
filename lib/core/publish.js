    ...
    return checkExist(deps)
            ...
            if (typeof existsRecord === 'object') {
                ...
                if (Object.keys(deps).length === Object.keys(exists).length) {
                    return Promise.reject({
                        errorno: 999,
                        deps: deps,
                        message: 'All components requested to publish already existed'
                    });
                }
                //TODO:we are manupulate deps here. NOT GOOD
                deps = narrowDown(deps, exists);
                return deps;
            }
            return true;
        })
        ...
        .then(function() {
            if (failedDeps.length > 0) {
                failedDeps = _.unique(failedDeps, function(n) {
        ...
        .then(function() {
            ...
            var failed = {}; //failedDeps is an array, need to convert to Object
            _.each(failedDeps, function(n) {
                failed[n.name] = n.version;
            });

            var installed = narrowDown(deps, failed);
            return {
                message: message,
                deps: depsOriginal,
                installed: installed,
        ...
        .catch(function(e) {
            if (e.errorno === 999) { //all exists
                ...
                var message = [
                    ...
                    'Make sure components to publish has been registered with ',
